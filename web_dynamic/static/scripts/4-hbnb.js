...
$(document).ready(function () {
    ...
    function searchPlacesWithAmenities() {
        const checkedAmenities = $('.amenities input[type="checkbox"]:checked').map(function() {
            return $(this).data('id');
        }).get();

        // Make a POST request to the places_search endpoint with checked amenities
        $.ajax({
    ...
    // Event listener for button click to search places with checked amenities
    $('button').click(function() {
        searchPlacesWithAmenities();
    });

    // Initially load places without filtering
    searchPlacesWithAmenities();
});
